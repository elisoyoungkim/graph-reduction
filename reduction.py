# original
import fuseCycle, IterativeDDLS, Queue, os
...
#     degree = reduce(lambda x,y: x+y, graph.values()).count(vertex) * len(successors)
#     return degree
...
#                 gen = IterativeDDLS.iterative_search(IterativeDDLS.Stack(), graph,  start_vertex, end_vertex, limit=limit)
    ...
    for path in list(dfs_path_set.union(bfs_path_set)):
        ...
        #print 'A set of paths in reduced subgraph G\' using Iterative DFS: {0}'.format(len(all_paths_of_fused_graph))
...
def connectivity(graph, start, end, prob_table, R_s, R_t, scc):
    ...
    if end not in R_s:
        ...
        #R_s = reachable(graph, start)
        
        #R_t = reachable(graph, end)
        if start == end:
            relevant_nodes = R_s
            anypath = []
...
def topol_dict(graph):
    ...
    #print 'TOPOL', topol_dictionary
    #for key, value in sorted(topol_dictionary.iteritems(), key=lambda (k,v): (v,k)): print "%s: %s" % (key, value)
        ...
        for edge in edges:
            ...
            if len(mergeable_edges) is 0:
                ...
                for medges in mergeable_edges:
                    ...
                    # 13|24 12|34 12|23
                    #print '#############', count, edge, mergeable_edges, medges     
                    ...
                    # if m0 < e0 < m1 or m0 < e1 < m1:
                    # #if m0 < e0 is between m0 and m1 or, e1 is between m0 and m1:
                    #     print edges[0], medges, edges[1], medges
                    #     if edge[0] in medges or edge[1] in medges:
                    #         count += 1
                    #         print '--'
                    #         print 'Edge {0} and edge {1} are mergeable'.format(edge[0]+edge[1], medges[0]+medges[1])
                    #     else:
                    ...
                    # else:
                    #     if edge[0] in medges or edge[1] in medges:
                    #         compare_nodes = list(set(edge).union(set(medges)).difference(set(edge).intersection(set(medges))))
                    #         if get_index(compare_nodes[0]) is get_index(compare_nodes[1]):
                    #             print '1'
                    #             count = count
                    #     elif len(set((edge[0], edge[1], medges[0], medges[1]))) != len(set((e0, e1, m0, m1))):
                    #         count = count
                    #         print '2'
                    #     else:
                    #         count += 1
...
# def merge(edges, graph, topol_dictionary):
#     
#     mergeable_edges_list = []
#     print 'hehehe',edges
#     while len(edges) is not 0:
#         mergeable_edges = define_mergeable_edges(edges, graph, topol_dictionary)
#         mergeable_edges_list.append(mergeable_edges)
#         print '1',mergeable_edges
#         print '2',mergeable_edges_list
#         for edge in mergeable_edges:
#             if edge in edges:
#                 edges.remove(edge)
#                 print 'edgessss', edges
#     print 'mergeable_edges_set',mergeable_edges_list
#     return mergeable_edges_list

# def get_shared_edges(edges_set, start_vertex, end_vertex, graph, topol_dictionary):
#     shared_edges_paths = []
#     #f = lambda x:x[0]
#     def get_index(node):
#         return topol_dictionary[node]
#     def relevent_node_set(start, end):
#         return set(reduce(lambda x,y:x+y, find_all_paths(graph, start, end)))
#     mergeable_paths_dictionary = {}
#     key_to_dict = []
#     for edges in edges_set:
#         temp = [edge[0]+edge[1] for edge in edges]
#         key_to_dict.append(tuple(temp))
#         mergeable_paths_dictionary.setdefault(tuple(temp), [])
#         mergeable = []
#         mergeable_paths = []
#         for edge in edges:
#             walk_path = []
#             final_path = []
#             for paths in find_all_paths(graph, edge[0], edge[1]):
#                 for start_path in find_all_paths(graph,start_vertex,edge[0]):
#                     if start_vertex is not edge[0]:
#                         walk_path.append(start_path[:-1] + paths)
#                     else:
#                         walk_path.append(paths)
#                 for walk in walk_path:
#                     for end_path in find_all_paths(graph,edge[1],end_vertex):
#                         if end_vertex is not edge[1]:
#                             final_path.append(walk+end_path[1:])
#                         else:
#                             final_path.append(walk)
#         [mergeable_paths.append(final) for final in final_path]
#         mergeable_paths_dictionary[tuple(temp)].extend(mergeable_paths)
#         #print 'Mergeable paths ',mergeable_paths
#         #print 'Mergeable paths dictionary ',mergeable_paths_dictionary
...
# #     relevant_neighbours = get_elimination_nodes(graph, start_vertex, end_vertex)
# #     print('R(s)-R(t)+{{s}}: {0}'.format(relevant_neighbours))
# #     for vertex in relevant_neighbours:
# #             print(vertex, relevant_neighbours)
# #             print('degree', vertex_degree(graph, vertex))
# #             if vertex_degree(graph, vertex) is 2:
# #                 print('sequence node',vertex)
# #                 for spath in shared_edges_paths:
# #                     
# #                 
#     print '+++', shared_edges_paths
# #     shared_edges = 
#     return (set.intersection(*shared_edges_paths), mergeable_paths_dictionary)
    
    

def get_shared_edges(bridge, edges_set, start_vertex, end_vertex, graph, topol_dictionary, prob_table, cyclenode_list, mode):
#     if len(bridge) is not 0:
#         bridge_node = reduce(lambda x,y:x+y, bridge)
#     else:
#         bridge_node = []
    #print 'bridge_node',bridge
    # for x in graph:
    #     print x, graph[x]
    shared_edges_paths = []
    #f = lambda x:x[0]
...
#                 and any(bridge not in path for bridge in bridge_node)
            ...
            for path_one in temp_part_one:
                ...
                if correct_path:
                    ...
                    #print '\np1', path_one
#             if nb_of_longest_paths > 1:
#                 bridge_node 
            #print 'Number of the longest paths in walk: ',nb_of_longest_paths
                    
                
                #walk_path = walk_path + walk_part_one
...
#             for walk in temp_rpath:
#                 correct_path = True
#                 for link in get_edges(walk):
#                     if abs(get_index(link[0])-get_index(link[1])) > 1:
#                         if (link[0], link[1]) != (edge[0], edge[1]):
#                             correct_path = False
#                     
...
#                 
    #print '+++', shared_edges_paths
#     shared_edges = 
    #print '@@@@@@@@@@',mergeable_paths_dictionary
    choice_node_set = []
    ...
    # def recursive_reduction(bridge, paths, cyclenode_list, graph, start_vertex, end_vertex, prob_table):
    #     # bridge = [(x1, x2, x3...),(y1, y2, y3...)]
    ...
    #     subpath = list(group_by_three_substring(paths))
    ...
    #                 subpath = list(group_by_three_substring(recursive_paths_list))
    ...
    #         [degree.extend([substring[1]]) for substring in subpath]
    #         #print 'degree', degree
    ...
    # return (recursive_paths_list, removed)
    # # def delete_choice_node(choice_node_set, mergeable_paths_dictionary):
...
#         return eliminate_choice_node(choice_node_set[0], mergeable_paths_dictionary)
    ...
    if len(choice_node_set) is not 0:
        while len(choice_node_set) is not 0:
            ...
            #print len(mergeable_paths_dictionary.values())
        
    #newest_mergeable_dict = mergeable_paths_dictionary
    #print 'newest',mergeable_paths_dictionary            
                    
#     for k in mergeable_paths_dictionary:
#         print k, mergeable_paths_dictionary[k]
...
# 
# def get_elimination_nodes(paths, start, end):
#     return list(set((reduce(lambda x,y:x+y, paths)))-set(start)-set(end))
